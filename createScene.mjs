...
export async function createScene(data, path) {
    ...
    const grouped = Object.groupBy(data.lieux ?? [], x => x.typeLieu)

    for (var i of Object.keys(grouped)) {
        materialHandler.setupMaterial(i);
    }

    for (var i of Object.keys(grouped)) {
        const material = materialHandler.getMaterial(i);
        const d = grouped[i].map(x => { return { x: x.x, y: x.z, z: x.y, data: x }; });
    ...
    scene.add(createInstancedMesh(smallBox, material_monstres, (data.monstres ?? []).map(x => { return { x: x.x + 0.1, y: x.z - 0.25, z: x.y, data: x }; })));
